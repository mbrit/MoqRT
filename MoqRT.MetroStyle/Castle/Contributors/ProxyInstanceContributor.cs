			...
			getObjectData.CodeBuilder.AddStatement(
				new AssignStatement(
					...
					new MethodInvocationExpression(
						...
						new ConstReference(typeof(ProxyObjectReference).Assembly()QualifiedName).ToExpression(),
			...
			for (var i = 0; i < interfaces.Length; i++)
			{
				getObjectData.CodeBuilder.AddStatement(
					new AssignArrayStatement(
						interfacesLocal,
						i,
						new ConstReference(interfaces[i].Assembly()QualifiedName).ToExpression()));
			}
			...
			getObjectData.CodeBuilder.AddStatement(
				new ExpressionStatement(
					new MethodInvocationExpression(
						...
						new ConstReference(emitter.BaseType.Assembly()QualifiedName).ToExpression())));
