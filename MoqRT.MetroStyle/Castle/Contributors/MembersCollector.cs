...
namespace Castle.DynamicProxy.Contributors
{
	...
	public abstract class MembersCollector
	{
        ...
        // @mbrit - 2012-06-04 - changed this... matching on the interfaces was failing...
		//private ICollection<IMethodInfo> checkedMethods = new HashSet<IMethodInfo>();
        private ICollection<string> checkedMethods = new HashSet<string>();
        
        private readonly IDictionary<IPropertyInfo, MetaProperty> properties = new Dictionary<IPropertyInfo, MetaProperty>();
		private readonly IDictionary<EventInfo, MetaEvent> events = new Dictionary<EventInfo, MetaEvent>();
		private readonly IDictionary<IMethodInfo, MetaMethod> methods = new Dictionary<IMethodInfo, MetaMethod>();
		...
		private void AddProperty(IPropertyInfo property, IProxyGenerationHook hook)
		{
		...
		{
			var addMethod = @event.GetAddMethod(true).AsIMethodInfo();
			var removeMethod = @event.GetRemoveMethod(true).AsIMethodInfo();
		...
		protected abstract MetaMethod GetMethodToGenerate(IMethodInfo method, IProxyGenerationHook hook, bool isStandalone);
		...
		private static bool IsInternalAndNotVisibleToDynamicProxy(IMethodInfo method)
		{
