...
namespace Castle.DynamicProxy.Contributors
{
	...
	{
		private readonly IList<IMethodInfo> methodsToSkip;
		private readonly Type targetType;

		public ClassProxyTargetContributor(Type targetType, IList<IMethodInfo> methodsToSkip, INamingScope namingScope)
			: base(namingScope)
		{
		...
		private Type BuildInvocationType(MetaMethod method, ClassEmitter @class, ProxyGenerationOptions options)
		{
			...
			var callback = CreateCallbackMethod(@class, IMethodInfo, method.MethodOnTarget);
			return new InheritanceInvocationTypeGenerator(callback.DeclaringType,
			                                              method,
			                                              callback.AsIMethodInfo(), null)
				.Generate(@class, options, namingScope)
				.BuildType();
		}

		private MethodBuilder CreateCallbackMethod(ClassEmitter emitter, IMethodInfo IMethodInfo, IMethodInfo methodOnTarget)
		{
			var targetMethod = methodOnTarget ?? IMethodInfo;
			var callBackMethod = emitter.CreateMethod(namingScope.GetUniqueName(IMethodInfo.Name + "_callback"), targetMethod);

			if (targetMethod.IsGenericMethod)
			{
				targetMethod = targetMethod.MakeGenericMethod(callBackMethod.GenericTypeParams.AsTypes().ToArray());
			}
		...
		private IInvocationCreationContributor GetContributor(Type @delegate, MetaMethod method)
		{
			...
			return new InvocationWithGenericDelegateContributor(@delegate,
			            method, new FieldReference(InvocationMethods.ProxyObject.AsFieldInfo()));
		}
