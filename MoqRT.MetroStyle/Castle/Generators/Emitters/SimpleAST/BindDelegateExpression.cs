...
namespace Castle.DynamicProxy.Generators.Emitters.SimpleAST
{
	...
	public class BindDelegateExpression : Expression
	{
		                              ...
		                              GenericTypeParameterBuilder[] genericTypeParams)
		{
			...
			{
				var closedDelegate = @delegate.MakeGenericType(genericTypeParams.AsTypes().ToArray());

                delegateCtor = TypeBuilder.GetConstructor(closedDelegate, delegateCtor);

				this.methodToBindTo = methodToBindTo.MakeGenericMethod(genericTypeParams.AsTypes().ToArray());
			}
			this.owner = owner;
		}
