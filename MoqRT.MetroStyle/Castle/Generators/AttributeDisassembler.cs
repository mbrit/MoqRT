...
namespace Castle.DynamicProxy.Generators
{
	...
	public class AttributeDisassembler : IAttributeDisassembler
	{
		...
		private static object[] GetPropertyValues(Type attType, out IPropertyInfo[] properties, Attribute original,
		                                          Attribute replicated)
		{
			...
			var selectedProperties = new List<IPropertyInfo>(propertyCandidates.Count);
		...
		{
			var properties = attributeType.GetProperties().AsIPropertyInfos();
		...
		///   we can convert it.
		/// </summary>
		private static IPropertyInfo ReplaceIfBetterMatch(ParameterInfo parameterInfo, IPropertyInfo propertyInfo,
		                                                 IPropertyInfo bestMatch)
		{
		...
		private static List<IPropertyInfo> GetPropertyCandidates(Type attributeType)
		{
			var propertyCandidates = new List<IPropertyInfo>();

			foreach (var pi in attributeType.GetProperties(BindingFlags.Instance | BindingFlags.Public).AsIPropertyInfos())
			{
