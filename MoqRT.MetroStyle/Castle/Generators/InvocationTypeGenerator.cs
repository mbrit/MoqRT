...
namespace Castle.DynamicProxy.Generators
{
	...
	public abstract class InvocationTypeGenerator : IGenerator<AbstractTypeEmitter>
	{
		...
		private readonly IMethodInfo callback;
		private readonly bool canChangeTarget;
		private readonly IInvocationCreationContributor contributor;

		protected InvocationTypeGenerator(Type targetType, MetaMethod method, IMethodInfo callback, bool canChangeTarget,
		...
		public AbstractTypeEmitter Generate(ClassEmitter @class, ProxyGenerationOptions options, INamingScope namingScope)
		{
			...
			var invocation = GetEmitter(@class, interfaces, namingScope, IMethodInfo);

			// invocation only needs to mirror the generic parameters of the IMethodInfo
			// targetType cannot be a generic type definition (YET!)
			invocation.CopyGenericParametersFromMethod(IMethodInfo);
			...
			ImplemementInvokeMethodOnTarget(invocation, IMethodInfo.GetParameters(), targetField, callback);
		...
		protected virtual MethodInvocationExpression GetCallbackMethodInvocation(AbstractTypeEmitter invocation,
		                                                                         Expression[] args, IMethodInfo callbackMethod,
		...
		{
			IConstructorInfo baseConstructor;
		...
		private IMethodInfo GetCallbackMethod(AbstractTypeEmitter invocation)
		{
			if (contributor != null)
			{
		...
		{
			var suggestedName = string.Format("Castle.Proxies.Invocations.{0}_{1}", IMethodInfo.DeclaringType.Name,
			                                  IMethodInfo.Name);
		...
		private void ImplemementInvokeMethodOnTarget(AbstractTypeEmitter invocation, ParameterInfo[] parameters,
		                                             FieldReference targetField, IMethodInfo callbackMethod)
		{
			...
			changeInvocationTarget.CodeBuilder.AddStatement(
				new ExpressionStatement(
					new ConvertExpression(@class.TypeBuilder, 
                        new FieldReference(InvocationMethods.ProxyObject.AsFieldInfo()).ToExpression())));
